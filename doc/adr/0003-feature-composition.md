...
As it should be easy to write features as pure functions (ADR 1), writing code to wire up all the dependencies would quickly become an issue where quants would spend time on making `ataraxia` work rather than focusing on their strategies.
...
Features composition fits nicely into ADR 1 pure functions decision, allowing for functional programming thinking within `ataraxia`.
